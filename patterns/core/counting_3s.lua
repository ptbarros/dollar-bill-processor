...
function match(ctx)
    ...
    -- Check if they form arithmetic sequence with step 3
    local valid = true
    for i = 1, 3 do
        if nums[i+1] - nums[i] ~= 3 then
            valid = false
            break
        end
    end
    ...
    return {
        ...
        connectors = {},
        message = string.format("Counting by 3s: %02d->%02d->%02d->%02d", nums[1], nums[2], nums[3], nums[4])
    }
end
