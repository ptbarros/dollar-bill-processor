--[[
Pattern: COUNTING_9S
DisplayName: Counting By 9s Ladder
...
function match(ctx)
    ...
    -- Check if they form arithmetic sequence with step 9
    local valid = true
    for i = 1, 3 do
        if nums[i+1] - nums[i] ~= 9 then
            valid = false
            break
        end
    end
    ...
    return {
        ...
        connectors = {},
        message = string.format("Counting by 9s: %02d->%02d->%02d->%02d", nums[1], nums[2], nums[3], nums[4])
    }
end
