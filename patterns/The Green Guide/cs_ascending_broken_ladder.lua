...
function match(ctx)
    ...
    -- Exclude full 8-digit ascending ladder (CS-1170)
    if is_ascending(d) then
        return {matched = false}
    end

    -- Find the longest consecutive ascending run (digit[i+1] == digit[i]+1)
    local best_start = -1
    local best_len = 0
    ...
    -- Must be length 3-7 (length 8 would be CS-1170, already excluded)
    if best_len < 3 then
        return {matched = false}
    end
    ...
    return {
        ...
        message = best_len .. "-digit ascending run at position " .. best_start .. " (CS-Ascending Broken Ladder)"
    }
end
