...
function match(ctx)
    ...
    if block_len ~= 4 then return {matched = false} end
    ...
    return {
        matched = true,
        group_boxes = {
            {from = base, to = base + 3, color = "lime", thickness = 3}
        },
        message = "4-digit descending ladder stand-alone (CS-Stand Alone Mini Down Ladder 4)"
    }
end
