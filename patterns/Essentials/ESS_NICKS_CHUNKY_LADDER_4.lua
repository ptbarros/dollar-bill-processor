--[[
Pattern: ESS_NICKS_CHUNKY_LADDER_4
DisplayName: 4 Digit Chunky Ladder
Description: 4 unique digits all in sorted order (ascending or descending)
Tier: 6
Examples: ["00001234", "01233333", "98760000", "44443210"]
--]]
...
function match(ctx)
    ...
    if table.concat(sorted_asc) == s then
        return {
            matched = true,
            message = "4-digit chunky ladder (ascending)",
    ...
    if table.concat(sorted_asc) == s then
        return {
            matched = true,
            message = "4-digit chunky ladder (descending)",
