--[[
Pattern: ESS_LOW_RUN_12M
DisplayName: Low Run 12.8M
Description: Match bills from 12.8 million BEP print runs
Tier: 6
...
function match(ctx)
    ...
    -- Look up in low runs data - only 12.8M runs
    for _, row in ipairs(ctx.data) do
        if row.series == series
           and row.district == district
           and row.block == block
           and row.facility == facility
           and row.quantity == "12.8" then
            return {
                matched = true,
                message = "Low run (12.8M): Series " .. series
                    .. " District " .. district
                    .. " Block " .. block
                    .. " " .. facility
            }
